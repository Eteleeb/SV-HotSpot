...
system ("mkdir -p $output_dir/processed_data");
...
print " Genes of interest     : $genes_of_int\n"; 
...
if ($output_folder ne '/data') {
   print "-------------------------------------------------------------------------------------------\n";
   print " SV-Hotspot pipeline finished successfully.\n"; 
   print " Results can be found at \"$output_folder\" at the local folder you provided with -v option\n";
   ...
   print "--------------------------------------------------------------------------------------------\n";
   print " SV-Hotspot pipeline finished successfully.\n";
   print " Results can be found at \"sv-hotspot-output\" at the local folder you provided with -v option\n";
   print " Started at $start\n";
   print " Results reported on $end\n";
   print "--------------------------------------------------------------------------------------------\n";
}
...
sub identify_peaks
{
   ...
   system ("detect-peaks.r $sv_type $chrom $peakPick_win $peakPick_minsd $pct_samples_t $output_dir $merge_dist $merge_pct_samples $stop_merge_num_peaks $genes_of_int $chromsize_file");
...
{
   print "\n------------------------------------------------------------------------------------\n";
   print "STEP 3: Determining the association between SVs and the expression of nearby genes\n";
   print "-------------------------------------------------------------------------------------\n";
...
sub visualize_res
{
   ...
   if ($expr_file & $cn_file) {
      system ("plot_peak_region.r $pks $output_dir $sv_file $output_dir $expr_file $cn_file $region_of_int $chip_cov $t_amp $t_del $chip_cov_lbl $left_ext $right_ext $genes_to_show $plot_layout");
   } elsif (!$expr_file) {
